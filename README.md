...
包含一份 **1335 条接口**的功能目录、三份逐字段核对过的接口契约文档，以及一套可复用的接口侦察方法。
...
5. **接口目录是"发现结果"，不是官方文档。** 站点随时可能改版，路径与字段会变。
...
| 接口目录 | 1335 条，按 19 个功能域分类，标注 HTTP 方法与「疑似写操作」 |
...
| [`docs/api-taxonomy.md`](docs/api-taxonomy.md) | 1335 条接口的功能分类（19 个功能域） |
...
├── yungu_api_catalog.md            1335 条接口目录（按功能域→模块）
