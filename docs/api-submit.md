...
1. **目录里没有** —— 全站 60+ 个 `delete` 接口，没有一个针对「我的提交里的某个文件」。
   最接近的两个都不是这个用途：
   - `GET /api/capture/deleteCaptureById` —— 删**整个成果**（连附件一起）
   - `GET /api/deleteResourcesFile` —— 课程资源，教师侧
2. **UI 上的 × 不发请求** —— 编辑态里点文件名旁的 `iconClose`，**一个请求都没发**，纯本地状态。
3. **真正的机制是「减项重交」** —— 重新提交一份**不含该文件**的 `fileList`，绑定即被替换。实测：

```
提交前绑定：  [11007501 fixed_probe.txt, 11007521 script_e2e.txt]
提交 fileList=[11007521]
提交后绑定：  [11007521 script_e2e.txt]        ← 11007501 从绑定中消失
```

> 服务端**按你给的 `fileList` 原样绑定**，不做合并。
> （应用会把历史附件带进去，是**前端**做的合并；本脚本默认复刻该行为，故需 `--only-new`
> 或下述新选项才能真正减项。）

### ⚠️ 这是「解绑」，不是「删除」

被移出绑定的文件**仍然存在于服务器**：实测 `11007501` 移出后仍可 `GET /api/preview_file?id=11007501`
读回 42 字节。

**文件本体没有删掉，只是不再属于那份提交。** 想真正删除文件字节，目前没有找到公开接口。

副作用：减项重交同样**新建成果版本**（`achievementId` 变化）。
