...
- **Cookie 即身份**，等同密码。别提交仓库、别贴群里。
  **服务端会话有效期未实测**——只测到 `SESSION` 本身是浏览器会话 cookie（无 `Expires`/`Max-Age`，关浏览器即失效），
  但服务端 TTL 未知（实测同一份 cookie 至少存活 6 天）。别把"过期重取即可"当成保证，长期不用的请主动退出登录。
- 除 `submit` 外，脚本只发**只读查询**（GET / 空 body 的查询类 POST），不含修改、删除任何操作。
  `submit` 是**唯一的写操作**：默认 dry-run，需 `--yes` + 交互输入 `yes` 才会发出，且提交前会校验身份字段完整性、提交后回读断言状态真的变了。
